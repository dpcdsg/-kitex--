...
import (
	...
	"github.com/ozline/tiktok/cmd/product/dal"
	"github.com/ozline/tiktok/cmd/product/rpc"
	"github.com/ozline/tiktok/config"
	product "github.com/ozline/tiktok/kitex_gen/product/productservice"
...
func Init() {
	...
	flag.Parse()
	config.Init(*path, constants.ProductServiceName)

	dal.Init()
	tracer.InitJaeger(constants.ProductServiceName)
...
func EsHookLog() *eslogrus.ElasticHook {
	hook, err := eslogrus.NewElasticHook(EsClient, config.Elasticsearch.Host, logrus.DebugLevel, constants.ProductServiceName)
	if err != nil {
		panic(err)
	}
...
func main() {
	...
	svr := product.NewServer(
		...
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{
			ServiceName: constants.ProductServiceName,
		}),
