...
import (
	...
	"github.com/ozline/tiktok/cmd/order/dal"
	"github.com/ozline/tiktok/cmd/order/rpc"
	"github.com/ozline/tiktok/config"
	order "github.com/ozline/tiktok/kitex_gen/order/orderservice"
...
func Init() {
	...
	flag.Parse()
	config.Init(*path, constants.OrderServiceName)

	dal.Init()
	tracer.InitJaeger(constants.OrderServiceName)
	rpc.Init()
...
func EsHookLog() *eslogrus.ElasticHook {
	hook, err := eslogrus.NewElasticHook(EsClient, config.Elasticsearch.Host, logrus.DebugLevel, constants.OrderServiceName)
	if err != nil {
		panic(err)
	}
	return hook
